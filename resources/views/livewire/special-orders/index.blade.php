            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-y border-gray-100 dark:border-gray-800">
                        <th class="py-3"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">رقم الطلب</p></th>
                        ...
                        <th class="py-3"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">السعر</p></th>
                        <th class="py-3"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">حالة الطلب</p></th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($this->orders as $order)
                        <tr wire:key="order-{{ $order->id }}">
                            <td class="py-3"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $loop->iteration }}</p></td>
                            <td class="py-3">
                                <div class="flex items-center gap-3">
                                    <div class="h-[50px] w-[50px] overflow-hidden rounded-md">
                                        <img src="{{ $order->driver->driver_image ? url($order->driver->driver_image) : asset('tailadmin/build/src/images/user/SO.jpg') }}" alt="Driver" />
                                    </div>
                                    <div>
                                        <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $order->driver->name }}</p>
                                        <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $order->driver->phone }}</span>
                                    </div>
                                </div>
                            </td>
                            <td class="py-3">
                            ...
                            <td class="py-3"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $order->price }}</p></td>
                            <td class="py-3">
                            ...
                            <td class="py-3">
                                <div class="flex items-center justify-center space-x-2">
                                    <p class="rounded-full bg-warning-50 px-2 py-0.5 text-theme-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-orange-400">{{ $order->start_address }}</p>
                                    ...
                                    <p class="rounded-full px-2 py-0.5 text-theme-xs font-medium bg-blue-light-50 text-blue-light-500 dark:bg-blue-light-500/15 dark:text-blue-light-500">{{ $order->end_address }}</p>
                                </div>
                            </td>
                            <td class="py-3">
                                <div class="flex items-center justify-center">
                                    {{-- Assuming you have a route for showing special order details --}}
                                    <a href="{{-- route('specialOrder.show', $order->id) --}}" class="flex items-center gap-1 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-theme-xs font-medium text-gray-700 transition-all hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
