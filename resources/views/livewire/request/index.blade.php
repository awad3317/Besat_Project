<div>
    ...
    <div class="flex flex-col sm:flex-row gap-4 md:gap-6 flex-wrap mb-4">
        ...
        <div wire:click.debounce.150ms="applyFilter('in_progress')" wire:loading.class="opacity-50" wire:target="applyFilter"
            ...
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-800">
                <svg fill="#dc6803" width="20" height="20" version="1.1" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="mt-3 w-full">
                ...
                <h4 class="mt-1 text-lg font-bold text-gray-800 dark:text-white/90">{{ $this->stats['in_progress'] ?? 0 }}</h4>
            </div>
        </div>
        ...
        <div wire:click.debounce.150ms="applyFilter('completed')" wire:loading.class="opacity-50" wire:target="applyFilter"
            ...
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-800">
                <svg width="20" height="20" viewBox="0 0 1024 1024" fill="#dc6803" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="mt-3 w-full">
                ...
                <h4 class="mt-1 text-lg font-bold text-gray-800 dark:text-white/90">{{ $this->stats['completed'] ?? 0 }}</h4>
            </div>
        </div>
        ...
        <div wire:click.debounce.150ms="applyFilter('cancelled')" wire:loading.class="opacity-50" wire:target="applyFilter"
            ...
            <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-gray-100 dark:bg-gray-800">
                <svg width="20" height="20" viewBox="0 0 512 512" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#dc6803">
            ...
            <div class="mt-3 w-full">
                ...
                <h4 class="mt-1 text-lg font-bold text-gray-800 dark:text-white/90">{{ $this->stats['cancelled'] ?? 0 }}</h4>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-y border-gray-100 dark:border-gray-800">
                        <th class="py-3 text-center"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">رقم الطلب</p></th>
                        <th class="py-3"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">السائق</p></th>
                        <th class="py-3"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">العميل</p></th>
                        <th class="py-3 text-center"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">السعر النهائي</p></th>
                        <th class="py-3 text-center"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">حالة الطلب</p></th>
                        <th class="py-3 text-center"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">من - الى</p></th>
                        <th class="py-3 text-center"><p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">الإجراءات</p></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($this->requests as $request)
                        <tr wire:key="request-{{ $request->id }}">
                            <td class="py-3"><p class="text-gray-500 text-center text-theme-sm dark:text-gray-400">{{ $request->id }}</p></td>
                            <td class="py-3">
                                @if($request->driver)
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $request->driver->name }}</p>
                                            <span class="text-gray-500 text-theme-xs dark:text-gray-400">{{ $request->driver->phone }}</span>
                                        </div>
                                    </div>
                                @else
                                    <span class="text-gray-400 text-sm">لم يتم التعيين بعد</span>
                                @endif
                            </td>
                            <td class="py-3">
                                @if($request->user)
                            ...
                            <td class="py-3"><p class="text-gray-500 text-center text-theme-sm dark:text-gray-400">{{ $request->final_price }}</p></td>
                            <td class="py-3">
                                <p class="rounded-full text-center px-2 py-0.5 text-theme-xs font-medium {{ $request->status_class }}">
                                    {{ $request->status_text }}
                                </p>
                            </td>
                            <td class="py-3">
                                <div class="flex items-center justify-center space-x-2">
                                    <p class="rounded-full bg-warning-50 px-2 py-0.5 text-theme-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-orange-400">{{ $request->start_address }}</p>
                                    ...
                                    <p class="rounded-full px-2 py-0.5 text-theme-xs font-medium bg-blue-light-50 text-blue-light-500 dark:bg-blue-light-500/15 dark:text-blue-light-500">{{ $request->end_address }}</p>
                                </div>
                            </td>
                            <td class="py-3">
                                <div class="flex items-center justify-center">
                                    {{-- افترض أن لديك رابط لعرض تفاصيل الطلب --}}
                                    <a href="{{-- route('requests.show', $request->id) --}}" class="flex items-center gap-1 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-theme-xs font-medium text-gray-700 transition-all hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
