<div x-data="{ isModalOpen: false, isLoading: false }">
    ...
    <div x-show="isModalOpen" class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto modal z-99999"
        ...
        <div @click.outside="isModalOpen = false"
            ...
            <form method="POST" action="{{ route('ads.store') }}" enctype="multipart/form-data" @submit="isLoading = true">
                @csrf
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-5">
                    ...
                    <div>
                        <label for="image" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                            صورة الإعلان <span class="mt-1 text-xs text-warning-500 dark:text-warning/90">*</span>
                        </label>
                        <input type="file" id="image" name="image" required accept="image/*"
                            class="hover:border-brand-500 dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-600 dark:text-white file:ml-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-brand-50 file:text-brand-700 hover:file:bg-brand-100 cursor-pointer">
