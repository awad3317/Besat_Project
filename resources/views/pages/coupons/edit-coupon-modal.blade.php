<div x-data="{ isModalEditOpen: @if(session('openModalEdit')) true @else false @endif, isLoading: false }">
    ...
    <div x-show="isModalEditOpen"
        ...
        <div @click.outside="isModalEditOpen = false"
            ...
            @endphp
            @if($Coupon)
                <form method="POST" action="{{ route('Coupon.update', $Coupon->id) }}" @submit="isLoading = true">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="number" id="discount_rate" name="discount_rate" required step="0.01" min="0.01"
                                ...
                                value="{{ number_format($Coupon->discount_rate * 100, 2) }}"
                                class="hover:border-brand-500 dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-600 dark:text-white">
                    ...
                    <div class="flex items-center justify-end w-full gap-3 mt-6">
                        ...
                        <button type="submit" :disabled="isLoading"
                            ...
                            <span x-text="isLoading ? 'جاري التعديل...' : 'تعديل الكوبون'"></span>
                        </button>
                    </div>
                </form>
            @endif
