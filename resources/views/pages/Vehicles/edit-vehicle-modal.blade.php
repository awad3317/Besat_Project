<div x-data="{ 
    ...
    imagePreview: null,
    isLoading: false,
    hasAc: '{{ session('vehicle') ? (old('has_ac_option', session('vehicle')->has_ac_option) ? '1' : '0') : '1' }}'
}">
                ...
                <form method="POST" action="{{ route('Vehicle.update', $vehicle->id) }}" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="number" step="0.01" min="0" placeholder="مثال: 25.00" name="min_price" required
                        ...
                        <div class="col-span-1" x-show="hasAc === '1'" x-transition>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                            ...
                            <input type="number" step="0.01" min="0" placeholder="مثال: 50.00" name="ac_price_per_km" 
                                :required="hasAc === '1'"
                                value="{{ old('ac_price_per_km', $vehicle->ac_price_per_km) }}"
                                class="hover:border-brand-500 dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-500 focus:ring-1 focus:ring-brand-500 dark:border-gray-600 dark:text-white">
                            @error('ac_price_per_km')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
